...
export function ActivityCard({ activity, onClick, onStatusChange, onSwipeDelete }: Props) {
  ...
  return (
    ...
    >
      <div
        onClick={onClick}
        className="group bg-white rounded-lg ring-1 ring-black/5 p-3 flex flex-col gap-2 cursor-pointer hover:ring-navy-600/30 transition-shadow"
      >
        ...
        <div className="flex items-center gap-3 pt-1" onClick={(e) => e.stopPropagation()}>
          ...
          {onSwipeDelete && (
            <button
              ...
              title="Excluir"
              className="ml-auto text-gray-300 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
            >
              🗑️
            </button>
          )}
